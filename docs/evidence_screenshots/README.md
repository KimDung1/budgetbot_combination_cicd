...
| `docs/evidence_screenshots/cost/02_sns_confirmed.png` | AWS Console -> SNS -> Topics -> `W7-Team14-BudgetAlerts` -> Subscriptions | Endpoint `truclt0311@gmail.com`, status confirmed/subscription ARN khong con PendingConfirmation |
...
| `docs/evidence_screenshots/security/05_cloudfront_https_acm.png` | AWS Console -> CloudFront -> Distribution `E349TWP6TRPSOC` -> General/Security | Alternate domain `budgetbot.topjob.id.vn`, ACM cert, TLSv1.2_2021 |
...
| `docs/evidence_screenshots/deployment/05_route53_records.png` | AWS Console -> Route 53 -> Hosted zone `topjob.id.vn` | A/AAAA alias for `budgetbot.topjob.id.vn` and ACM validation record |
| `docs/evidence_screenshots/deployment/06_api_gateway_routes.png` | AWS Console -> API Gateway -> API `mlhwir8gc5` -> Routes | `/upload`, `/summary`, `/transactions`, `/review-queue`, `/accuracy`, `/stats`, `/correct`, `/health` |
...
| File path | Chup o dau | Noi dung can thay |
|---|---|---|
| `docs/evidence_screenshots/demo/01_live_url_home.png` | Browser -> `https://budgetbot.topjob.id.vn` | App loads from public HTTPS URL |
